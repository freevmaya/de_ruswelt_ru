	...
	class Jet_Blog_Settings {
		...
		public function init() {
			...
			foreach ( glob( jet_blog()->plugin_path( 'includes/addons/' ) . '*.php' ) as $file ) {
				...
				$this->available_widgets[ $slug ] = $data['name'];
			}
		...
		public function register_controls() {
			...
			$default_available_widgets = array();

			foreach ( $this->available_widgets as $key => $value ) {
				$default_available_widgets[ $key ] = 'true';
			}
			...
			$this->builder->register_control(
				array(
					'available_widgets' => array(
						'type'        => 'checkbox',
						'id'          => 'available_widgets',
						'name'        => 'available_widgets',
						'parent'      => 'settings_top',
						'value'       => $this->get( 'available_widgets', $default_available_widgets ),
						'options'     => $this->available_widgets,
						'title'       => esc_html__( 'Available Widgets', 'jet-blog' ),
						'description' => esc_html__( 'List of widgets that will be available when editing the page', 'jet-blog' ),
						'class'       => 'jet_blog_settings_form__checkbox-group',
					),
				)
			);
			...
			$this->builder->register_control(
				array(
					'widgets_load_level' => array(
						...
						'title'       => esc_html__( 'Editor Load Level', 'jet-blog' ),
						'description' => esc_html__( 'Choose a certain set of options in the widget’s Style tab by moving the slider, and improve your Elementor editor performance by selecting appropriate style settings fill level (from None to Full level)', 'jet-blog' ),
					),
				)
			);
