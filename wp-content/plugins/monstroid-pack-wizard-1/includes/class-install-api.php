...
if ( ! class_exists( 'MPack_Wizard_Install_API' ) ) {

	/**
	 * Define MPack_Wizard_Install_API class
	 */
	class MPack_Wizard_Install_API {
		...
		public function do_theme_install() {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'The theme is succesfully installed. Activating...', 'monstroid-pack-wizard' );
			...
			if ( is_wp_error( $result ) ) {
				...
				global $wp_filesystem;
				$message = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'monstroid-pack-wizard' );
		...
		public function do_plugin_install( $slug = null ) {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'Plugins wizard installed and activated. Redirecting...', 'monstroid-pack-wizard' );
		...
		public function adjust_plugin_dir( $source, $remote_source, $upgrader ) {
			...
			if ( empty( $_REQUEST['action'] ) && 'mpack_wizard_install_plugins_wizard' !== $_REQUEST['action'] ) {
				return $source;
			}
		...
		public function adjust_theme_dir( $source, $remote_source, $upgrader ) {
			...
			// Ensure that is Wizard installation request
			if ( empty( $_REQUEST['action'] ) && 'mpack_wizard_install_parent' !== $_REQUEST['action'] ) {
				return $source;
			}
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				...
				return new WP_Error(
					'rename_failed',
					esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'monstroid-pack-wizard' ),
...
/**
 * Returns instance of MPack_Wizard_Install_API
 *
 * @return object
 */
function mpack_install_api( $url = null ) {
	return MPack_Wizard_Install_API::get_instance( $url );
}
