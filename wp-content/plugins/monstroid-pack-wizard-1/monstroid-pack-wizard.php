<?php
/*
Plugin Name: Monstroid Pack Wizard
Plugin URI:
Description: Monstroid package installation wizard.
Author: Zemez
Author URI:
Version: 1.1.0
Text Domain: monstroid-pack-wizard
...
if ( ! class_exists( 'MPack_Wizard' ) ) {

	/**
	 * Define MPack_Wizard class
	 */
	class MPack_Wizard {
		 ...
		 * @var string
		 */
		private $slug = 'mpack-wizard';

		/**
		 * Nonce action name.
		 * @var string
		 */
		public $_nonce = 'mpack-wizard-nonce';
		...
		private $files = array(
			...
			'updater-api' => 'includes/class-updater-api.php',
		...
		function __construct() {
			...
			$this->settings = array(
				'options' => array(
					'parent_data' => 'mpack-wizard-installed-parent',
					'child_data'  => 'mpack-wizard-installed-child',
				),
			);
		...
		public function hooks() {
			...
			add_action( 'admin_menu', array( mpack_interface(), 'register_page' ) );
			add_action( 'admin_menu', array( mpack_interface(), 'register_page' ) );
		}
		...
		public function lang() {
			load_plugin_textdomain( 'monstroid-pack-wizard', false, $this->path( 'languages' ) );
		}
		...
		public function activation_redirect() {
			...
			$enbled = apply_filters( 'mpack-wizard/activation-redirect-enabled', $enbled );
			...
			delete_transient( $this->slug() . '_redirect' );
			wp_redirect( mpack_interface()->get_page_link() );
			die();
		}
		...
		public function enqueue_assets( $hook ) {
			...
			wp_localize_script( $this->slug(), 'MPackWizardSettings', array(
				'nonce'  => wp_create_nonce( $this->slug() ),
				'theme'  => isset( $_GET['theme'] ) ? esc_attr( $_GET['theme'] ) : '',
				'errors' => array(
					'empty' => esc_html__( '* Please, fill this field', 'monstroid-pack-wizard' ),
				),
			) );
			 ...
			 */
			do_action( 'mpack-wizard/enqueue-assets' );
		}
...
/**
 * Returns instance of MPack_Wizard
 *
 * @return object
 */
function mpack_wizard() {
	return MPack_Wizard::get_instance();
}
