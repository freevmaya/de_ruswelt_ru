 ...
 * @return array
 */
function mortgates_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! mortgates_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'mortgates_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'mortgates_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'mortgates' ) . $network['name'],
	...
	$share_before_html = apply_filters( 'mortgates_share_toggle_html', null );
