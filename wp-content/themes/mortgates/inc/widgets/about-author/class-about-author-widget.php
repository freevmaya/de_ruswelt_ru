 ...
 * @package Mortgates
 */
if ( ! class_exists( 'Mortgates_About_Author_Widget' ) ) {

	/**
	 * Class Mortgates_About_Author_Widget.
	 */
	class Mortgates_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'mortgates' );
			$this->widget_id          = 'mortgates_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'mortgates' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'mortgates' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'mortgates' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'mortgates' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'mortgates' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'mortgates' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'mortgates' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'mortgates' ),
					'label' => esc_html__( 'Link label', 'mortgates' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'mortgates' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'mortgates-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = mortgates_get_locate_template( 'inc/widgets/about-author/view/about-author.php' );
...
add_action( 'widgets_init', 'mortgates_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function mortgates_register_about_author_widgets() {
	register_widget( 'Mortgates_About_Author_Widget' );
}
