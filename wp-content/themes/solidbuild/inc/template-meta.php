 ...
 * @return array
 */
function solidbuild_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! solidbuild_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$defaults = apply_filters( 'solidbuild_default_args_share_buttons', array(
		...
		'pinterest' => array(
			...
			'name'      => esc_html__( 'Pinterest', 'solidbuild' ),
	...
	$default_config = apply_filters( 'solidbuild_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'solidbuild_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'solidbuild' ) . $network['name'],
 ...
 * @return string
 */
function solidbuild_get_single_share_buttons() {
	ob_start();
