...
add_filter( 'solidbuild_get_dynamic_css_options', 'solidbuild_add_elementor_dynamic_file' );

// Add custom icon support.
add_filter( 'jet-elements/controls/icon/data', 'solidbuild_set_theme_icons' );
add_action( 'elementor/editor/after_enqueue_styles', 'solidbuild_enqueue_font' );
add_action( 'elementor/frontend/after_register_styles', 'solidbuild_enqueue_font' );

// Add custom class at columns if paddings are used
add_action( 'elementor/frontend/element/before_render', 'solidbuild_add_column_additional_classes' );

// Add custom class at tabs if border are not used
add_action( 'elementor/widget/render_content', 'solidbuild_rewrite_tabs_render_template', 10, 2 );
add_action( 'elementor/widget/print_template', 'solidbuild_rewrite_tabs_print_template', 10, 2 );

add_action( 'elementor/element/tabs/section_tabs/after_section_end', 'solidbuild_update_tabs_controls' );

/**
 * Add builder dynamic files.
 */
function solidbuild_add_elementor_dynamic_file( $options ) {

	$dynamic_files = array(
		SOLIDBUILD_THEME_DIR . '/assets/css/dynamic/plugins/elementor.css',
	);
 ...
 * Add icon styles in admin.
 */
function solidbuild_enqueue_font() {
 ...
 * Add custom icon support.
 */
function solidbuild_set_theme_icons( $data ) {
	return array(
 ...
 */
function solidbuild_add_column_additional_classes( \Elementor\Element_Base $element ) {
 ...
 */
function solidbuild_rewrite_tabs_print_template( $template, $widget ) {
 ...
 */
function solidbuild_rewrite_tabs_render_template( $element, $data ) {
	...
	<div class="<?php echo $wrap_class; ?>" role="tablist">
		...
		<div class="elementor-tabs-content-wrapper" role="tabpanel">
			<?php foreach ( $tabs as $item ) : ?>
				...
				<div class="elementor-tab-content elementor-clearfix" data-tab="<?php echo $counter; ?>"><?php echo solidbuild_parse_text_editor( $item['tab_content'], $data ); ?></div>
			<?php
				$counter++;
			endforeach; ?>
		</div>
...
function solidbuild_parse_text_editor( $content, $data ) {
 ...
 */
function solidbuild_update_tabs_controls( $widget ) {
	...
	$widget->update_control(
		...
		array(
			'fields' => array(
				array(
					'name'        => 'tab_icon',
					'label'       => esc_html__( 'Icon', 'solidbuild' ),
					'type'        => Elementor\Controls_Manager::ICON,
					'label_block' => true,
				),
				array(
					'name'        => 'tab_title',
					'label'       => esc_html__( 'Title & Content', 'solidbuild' ),
					'type'        => Elementor\Controls_Manager::TEXT,
					'default'     => esc_html__( 'Tab Title', 'solidbuild' ),
					'placeholder' => esc_html__( 'Tab Title', 'solidbuild' ),
					'label_block' => true,
				),
				array(
					'name'        => 'tab_content',
					'label'       => esc_html__( 'Content', 'solidbuild' ),
					'default'     => esc_html__( 'Tab Content', 'solidbuild' ),
					'placeholder' => esc_html__( 'Tab Content', 'solidbuild' ),
	...
	$widget->update_control(
		...
		array(
			'options' => [
				...
				'icon_horizontal' => esc_html__( 'Horizontal With Icon', 'solidbuild' ),
				'icon_vertical'   => esc_html__( 'Vertical With Icon', 'solidbuild' ),
			],
		)
	);
