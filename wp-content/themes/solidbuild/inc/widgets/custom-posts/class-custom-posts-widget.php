...
if ( ! class_exists( 'Solidbuild_Custom_Posts_Widget' ) ) {

	/**
	 * Class Solidbuild_Custom_Posts_Widget.
	 */
	class Solidbuild_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'solidbuild' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'solidbuild' );
			$this->widget_id          = apply_filters( 'solidbuild_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'solidbuild_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = solidbuild_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'solidbuild' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'solidbuild' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'solidbuild' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'solidbuild' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'solidbuild' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'solidbuild' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'solidbuild' ),
						'category'      => esc_html__( 'Category', 'solidbuild' ),
						'post_tag'      => esc_html__( 'Tag', 'solidbuild' ),
						'more_button'   => esc_html__( 'More Button', 'solidbuild' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'solidbuild' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = solidbuild_get_locate_template( 'inc/widgets/custom-posts/views/custom-post-view.php' );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', solidbuild_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'solidbuild-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'solidbuild_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function solidbuild_register_custom_posts_widget() {
		register_widget( 'Solidbuild_Custom_Posts_Widget' );
	}
}
