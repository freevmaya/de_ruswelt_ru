 ...
 * @package Solidbuild
 */
if ( ! class_exists( 'Solidbuild_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Solidbuild_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'solidbuild' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'solidbuild' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'solidbuild' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'solidbuild' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'solidbuild' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'solidbuild' ),
						'title'         => esc_html__( 'Post Title', 'solidbuild' ),
						'comment_count' => esc_html__( 'Comment Count', 'solidbuild' ),
					),
					'label'   => esc_html__( 'Post sorted', 'solidbuild' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'solidbuild' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'solidbuild' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'solidbuild' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'solidbuild_image_grid_widget_args', $args, $instance );

			if ( $this->get_cached_widget( $args ) ) {
				return;
			}

			$template = solidbuild_get_locate_template( '/inc/widgets/image-grid/views/image-grid-view.php' );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'solidbuild_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'solidbuild_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'solidbuild_image_grid_widget_before',
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'              => 'solidbuild-thumb-m',
						'mobile_size'       => 'solidbuild-thumb-m',
				...
				echo apply_filters( 'solidbuild_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'solidbuild_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function solidbuild_register_image_grid_widget() {
		register_widget( 'Solidbuild_Image_Grid_Widget' );
	}
}
