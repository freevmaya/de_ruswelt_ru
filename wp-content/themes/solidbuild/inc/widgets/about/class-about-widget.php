...
if ( ! class_exists( 'Solidbuild_About_Widget' ) ) {

	/**
	 * Class Solidbuild_About_Widget.
	 */
	class Solidbuild_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'solidbuild' );
			$this->widget_id          = 'solidbuild_widget_about';
			$this->widget_name        = esc_html__( 'About Solidbuild', 'solidbuild' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'solidbuild' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'solidbuild' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'solidbuild' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'solidbuild' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'solidbuild' ),
					'value'             => '',
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, solidbuild_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = solidbuild_get_locate_template( 'inc/widgets/about/views/about.php' );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'solidbuild_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'solidbuild_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'solidbuild_register_about_widget' );
/**
 * Register about widget.
 */
function solidbuild_register_about_widget() {
	register_widget( 'Solidbuild_About_Widget' );
}
