...
if ( ! class_exists( 'Solidbuild_Smart_Slider_Widget' ) ) {

	/**
	 * Class Solidbuild_Smart_Slider_Widget.
	 */
	class Solidbuild_Smart_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display smart slider on your site.', 'solidbuild' );
			$this->widget_id          = 'solidbuild_widget_smart_slider';
			$this->widget_name        = esc_html__( 'Smart Slider', 'solidbuild' );
			...
			$this->settings           = array(
				...
				'tags' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'solidbuild' ),
					'multiple'         => true,
				...
				'post_title' => array(
					...
					'label' => esc_html__( 'Display title', 'solidbuild' ),
				),
				'content' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display content', 'solidbuild' ),
				),
				'more_button' => array(
					'type'   => 'switcher',
					'value'  => 'true',
				...
				'title_trim_words' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title words trimmed count', 'solidbuild' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
				...
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width', 'solidbuild' ),
				),
				'height' => array(
					'type'  => 'text',
					'value' => '800',
					'label' => esc_html__( 'Slider height', 'solidbuild' ),
				),
				'orientation' => array(
					'type'    => 'select',
					'size'    => 1,
					'value'   => 'horizontal',
					'options' => array(
						'horizontal' => esc_html__( 'Horizontal', 'solidbuild' ),
						'vertical'   => esc_html__( 'Vertical', 'solidbuild' ),
					),
					'label'   => esc_html__( 'Slider orientation', 'solidbuild' ),
				),
				'slide_distance' => array(
					...
					'max_value' => 100,
					'min_value' => 0,
					'label'     => esc_html__( 'Slide distance(px)', 'solidbuild' ),
				),
				'slide_duration' => array(
					...
					'min_value'  => 100,
					'step_value' => 100,
					'label'      => esc_html__( 'Slide duration(ms)', 'solidbuild' ),
				),
				'slide_fade' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use fade effect?', 'solidbuild' ),
				),
				'navigation' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Use navigation?', 'solidbuild' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'solidbuild' ),
						'false_toggle' => esc_html__( 'Off', 'solidbuild' ),
				...
				'fade_navigation' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'solidbuild' ),
				...
				'pagination' => array(
					...
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use pagination?', 'solidbuild' ),
				),
				'autoplay' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use autoplay?', 'solidbuild' ),
				),
				'fullScreen' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display fullScreen button?', 'solidbuild' ),
				),
				'shuffle' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Indicates if the slides will be shuffled', 'solidbuild' ),
				),
				'loop' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use infinite scrolling?', 'solidbuild' ),
				),
				'thumbnails' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails?', 'solidbuild' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'solidbuild' ),
						'false_toggle' => esc_html__( 'Off', 'solidbuild' ),
				...
				'thumbnails_position' => array(
					...
					'options' => array(
						...
						'bottom' => esc_html__( 'Bottom', 'solidbuild' ),
						'right'  => esc_html__( 'Right', 'solidbuild' ),
						'left'   => esc_html__( 'Left', 'solidbuild' ),
					),
					'label'   => esc_html__( 'Sets the position of the thumbnail scroller', 'solidbuild' ),
				...
				'thumbnails_arrows' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails arrows?', 'solidbuild' ),
					'master' => 'thumbnails_attr',
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $posts_query ) {
				...
				$html = '<h4>' . esc_html__( 'Posts for slider not found', 'solidbuild' ) . '</h4>';
			}
		...
		public function render_slider( $posts_query ) {
			...
			$html = '<div class="solidbuild-smartslider smart-slider__instance invert" ' . $slider_html_attr . '>';
		...
		public function get_query_slider_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'solidbuild_smart_slider_default_query_args', array(
				'post_type'           => 'post',
				'orderby'             => 'date',
				'order'               => 'DESC',
				'posts_per_page'      => - 1,
		...
		public function get_slider_loop( $posts_query ) {
			 ...
			 * @var  array
			 */
			$title_settings = apply_filters( 'solidbuild_smart_slider_title_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '38%%',
			 ...
			 * @var  array
			 */
			$content_settings = apply_filters( 'solidbuild_smart_slider_content_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '51%%',
				'show_transition' => 'left',
			 ...
			 * @var  array
			 */
			$more_settings = apply_filters( 'solidbuild_smart_slider_more_button_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '58%%',
				'show_transition' => 'left',
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$html .= '<div class="smart-slider__item sp-slide">';
					$html .= $this->utility->media->get_image( array(
						'size'        => 'solidbuild-thumb-xl',
						'mobile_size' => 'solidbuild-thumb-l',
						'class'       => 'sp-image',
		...
		public function get_slider_thumbnails( $posts_query ) {
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				$html .= '<div class="sp-thumbnail">';
					$html .= $this->utility->media->get_image( array(
						'size'        => 'solidbuild-slider-thumb',
						'mobile_size' => 'solidbuild-slider-thumb',
	...
	add_action( 'widgets_init', 'solidbuild_register_smart_slider_widgets' );
	/**
	 * Register Smart slider widget.
	 */
	function solidbuild_register_smart_slider_widgets() {
		register_widget( 'Solidbuild_Smart_Slider_Widget' );
	}
}
