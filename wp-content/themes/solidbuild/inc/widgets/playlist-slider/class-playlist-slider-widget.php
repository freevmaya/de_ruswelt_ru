...
if ( ! class_exists( 'Solidbuild_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Solidbuild_Playlist_Slider_Widget.
	 */
	class Solidbuild_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'solidbuild' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'solidbuild' );
			$this->widget_id          = apply_filters( 'solidbuild_playlist_slider_widget_ID', 'solidbuild_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'solidbuild_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = solidbuild_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'solidbuild' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'solidbuild' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'solidbuild' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '525',
					'label' => esc_html__( 'Slider height ( px, rem )', 'solidbuild' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'solidbuild' ),
						true  => esc_html__( 'Buttons', 'solidbuild' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'solidbuild' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'solidbuild' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'solidbuild' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'solidbuild' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'solidbuild' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'solidbuild' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'solidbuild' ),
						'author'   => esc_html__( 'Author', 'solidbuild' ),
						'comments' => esc_html__( 'Comment count', 'solidbuild' ),
						'category' => esc_html__( 'Category', 'solidbuild' ),
						'post_tag' => esc_html__( 'Tag', 'solidbuild' ),
					),
					'label' => esc_html__( 'Display post meta data', 'solidbuild' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'solidbuild' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'solidbuild_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			$thumbnails_view_dir = solidbuild_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-thumbnails-view.php' );
			...
			foreach ( $this->posts as $post ) {
				...
				solidbuild_theme()->dynamic_css->add_style(
		...
		public function the_slides_view() {
			$slides_view_dir = solidbuild_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-slides-view.php' );
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'solidbuild_playlist_buttons', false ),
				...
				$holder_view_dir = solidbuild_get_locate_template( 'inc/widgets/playlist-slider/views/playlist-holder-view.php' );
...
add_action( 'widgets_init', 'solidbuild_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function solidbuild_register_playlist_slider_widget() {
	register_widget( 'Solidbuild_Playlist_Slider_Widget' );
}
