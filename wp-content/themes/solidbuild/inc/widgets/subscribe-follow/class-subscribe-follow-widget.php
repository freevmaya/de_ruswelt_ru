...
class Solidbuild_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'solidbuild' );
		$this->widget_id          = 'solidbuild_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'solidbuild' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'solidbuild' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'solidbuild' ),
				'label' => esc_html__( 'Subscribe Title', 'solidbuild' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'solidbuild' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'solidbuild' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'solidbuild' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'solidbuild' ),
				'label' => esc_html__( 'Subscribe submit label', 'solidbuild' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'solidbuild' ),
				'label' => esc_html__( 'Subscribe success', 'solidbuild' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'solidbuild' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'solidbuild' ),
				'label' => esc_html__( 'Follow Title', 'solidbuild' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'solidbuild' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'solidbuild' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'solidbuild' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'solidbuild' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'solidbuild' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'solidbuild' ),
				'value'   => 'center',
				'options' => array(
			...
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'solidbuild' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'solidbuild' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'solidbuild' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'solidbuild' ),
				'value'   => 'inherit',
				'options' => array(
			...
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'solidbuild' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_solidbuild_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_solidbuild_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'solidbuild' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'solidbuild' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'solidbuild' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'solidbuild' ),
		);

		$this->nonce = wp_create_nonce( 'solidbuild-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = solidbuild_get_locate_template( 'inc/widgets/subscribe-follow/view/follow-view.php' );
		$subscribe_template         = solidbuild_get_locate_template( 'inc/widgets/subscribe-follow/view/subcribe-view.php' );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'solidbuild' );
		}
	...
	public function get_social_nav() {
		return solidbuild_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = solidbuild_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'solidbuild-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'solidbuild_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function solidbuild_register_subscribe_follow_widgets() {
	register_widget( 'Solidbuild_Subscribe_Follow_Widget' );
}
