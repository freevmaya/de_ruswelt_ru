...
if ( ! class_exists( 'Solidbuild_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Solidbuild_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'solidbuild' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'solidbuild' );
			$this->widget_id          = 'solidbuild-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'solidbuild' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'solidbuild' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'solidbuild' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'solidbuild' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'solidbuild' ),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'solidbuild' ),
				...
				'tiles_type' => array(
					...
					'options' => array(
						'tiles-layout-1' => array(
							'label' => esc_html__( 'Layout 1', 'solidbuild' ),
						),
						'tiles-layout-2' => array(
							'label' => esc_html__( 'Layout 2', 'solidbuild' ),
						),
					),
		...
		public function widget( $args, $instance ) {
			...
			$template = solidbuild_get_locate_template( 'inc/widgets/taxonomy-tiles/views/taxonomy-tiles-view.php' );
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'solidbuild_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'solidbuild_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'solidbuild' ),
					...
					if ( 'grid' === $layout_type ) {
						...
						$bg_image_url = $this->utility->media->get_image( array(
							'html'        => '%3$s',
							'size'        => 'solidbuild-thumb-760-571',
							'mobile_size' => 'solidbuild-thumb-760-571',
						), 'term', $term->term_id );
				...
				echo apply_filters( 'solidbuild_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'solidbuild_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function solidbuild_register_taxonomy_tiles_widget() {
		register_widget( 'Solidbuild_Taxonomy_Tiles_Widget' );
	}
}
