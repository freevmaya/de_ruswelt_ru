...
/**
 * Class Solidbuild_News_Smart_Box_Widget
 */
class Solidbuild_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'solidbuild_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'solidbuild' );
		$this->widget_description = esc_html__( 'News Smart Box', 'solidbuild' );
		$this->utility            = solidbuild_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'solidbuild' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'solidbuild' ),
						'img_src' => SOLIDBUILD_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'solidbuild' ),
						'img_src' => SOLIDBUILD_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'solidbuild' ),
						'img_src' => SOLIDBUILD_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'solidbuild' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
				'options' => array(
					'category' => array(
						'label' => esc_html__( 'Category', 'solidbuild' ),
						'slave' => 'terms_type_category',
					),
					'post_tag' => array(
						'label' => esc_html__( 'Tag', 'solidbuild' ),
						'slave' => 'terms_type_post_tag',
					),
				),
				'label' => esc_html__( 'Choose taxonomy type', 'solidbuild' ),
			),
			'current_category' => array(
				...
				'options_callback' => array( $this, 'get_terms_list',
					array( 'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select category', 'solidbuild' ),
				'placeholder'      => esc_html__( 'Select category', 'solidbuild' ),
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'solidbuild' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'solidbuild' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'solidbuild' ),
				'placeholder'      => esc_html__( 'Select tag', 'solidbuild' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'solidbuild' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select tags', 'solidbuild' ),
				'master'           => 'terms_type_post_tag',
			),
			'posts_per_page' => array(
				...
				'max_value' => 20,
				'min_value' => 1,
				'label'     => esc_html__( 'Posts count', 'solidbuild' ),
			),
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'solidbuild' ),
			),
			'trim_content_words' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Content words trimmed count', 'solidbuild' ),
			),
			'current_title_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display current navigation title', 'solidbuild' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'solidbuild' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'solidbuild' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'solidbuild' ),
			),
			'category_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display categories', 'solidbuild' ),
			),
			'tags_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display tags', 'solidbuild' ),
			),
			'more_button_visibility' => array(
				'type'   => 'switcher',
				'value'  => 'true',
				'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label'  => esc_html__( 'Display more button', 'solidbuild' ),
				'toggle' => array(
					'true_toggle'  => esc_html__( 'On', 'solidbuild' ),
					'false_toggle' => esc_html__( 'Off', 'solidbuild' ),
			...
			'more_button_text' => array(
				'type'   => 'text',
				'value'  => esc_html__( 'Read more', 'solidbuild' ),
				'label'  => esc_html__( 'More button text', 'solidbuild' ),
		...
		add_filter( 'solidbuild_theme_script_variables', array( $this, 'add_new_smart_box_nonce_var' ) );
	...
	public function add_new_smart_box_nonce_var( $vars = array() ) {
		$vars['new_smart_box_nonce'] = wp_create_nonce( 'solidbuild-new-smart-box-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$loop_template = solidbuild_get_locate_template( 'inc/widgets/news-smart-box/views/loop-view.php' );
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'solidbuild_news_smart_box_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => 10,
	...
	public function get_smart_box_loop( $posts_query ) {
		$full_view_template = solidbuild_get_locate_template( 'inc/widgets/news-smart-box/views/full-view.php' );
		$mini_view_template = solidbuild_get_locate_template( 'inc/widgets/news-smart-box/views/mini-view.php' );
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'solidbuild' ) . '</h4></div>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$author = $this->utility->meta_data->get_author(array(
				...
				'prefix'  => esc_html__( 'by ', 'solidbuild' ),
			...
			$category = $this->utility->meta_data->get_terms( array(
				...
				'after'   => '</span>',
				'prefix'  => esc_html__( 'in ', 'solidbuild' ),
			) );
			...
			$tags = $this->utility->meta_data->get_terms( array(
				...
				'type'      => 'post_tag',
				'delimiter' => ', ',
				'prefix'    => esc_html__( 'Tags: ', 'solidbuild' ),
			...
			$image_size      = 'solidbuild-thumb-560-315';
			$title_tag       = 'h4';
			...
			switch ( $this->instance['layout_type'] ) {
				case 'layout_type_1':
					if ( 1 <= $posts_query->current_post ) {
						...
						$image_size      = 'solidbuild-thumb-260-147';
						$title_tag       = 'h5';
					} else {
	...
	public function new_smart_box_instance(){
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'solidbuild-new-smart-box-nonce' ) ) {
			exit();
		}
...
add_action( 'widgets_init', 'solidbuild_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function solidbuild_register_news_smart_box_widgets() {
	register_widget( 'Solidbuild_News_Smart_Box_Widget' );
}
