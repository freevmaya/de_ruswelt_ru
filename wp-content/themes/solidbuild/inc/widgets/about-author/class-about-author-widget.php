 ...
 * @package Solidbuild
 */
if ( ! class_exists( 'Solidbuild_About_Author_Widget' ) ) {

	/**
	 * Class Solidbuild_About_Author_Widget.
	 */
	class Solidbuild_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'solidbuild widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'solidbuild' );
			$this->widget_id          = 'solidbuild_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'solidbuild' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'solidbuild' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'solidbuild' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'solidbuild' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'solidbuild' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'solidbuild' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'solidbuild' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'solidbuild' ),
					'label' => esc_html__( 'Link label', 'solidbuild' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'solidbuild' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'solidbuild-thumb-m' );
		...
		public function widget( $args, $instance ) {
			...
			$template = solidbuild_get_locate_template( 'inc/widgets/about-author/view/about-author.php' );
...
add_action( 'widgets_init', 'solidbuild_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function solidbuild_register_about_author_widgets() {
	register_widget( 'Solidbuild_About_Author_Widget' );
}
