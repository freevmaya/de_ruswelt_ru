...
if ( ! class_exists( 'Solidbuild_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Solidbuild_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'solidbuild' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'solidbuild' );
			$this->widget_id          = 'solidbuild_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => SOLIDBUILD_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'solidbuild' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'solidbuild' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'solidbuild' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'solidbuild' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'solidbuild' ),
						'categories' => esc_html__( 'Show categories', 'solidbuild' ),
						'tags'       => esc_html__( 'Show tags', 'solidbuild' ),
						'author'     => esc_html__( 'Show author', 'solidbuild' ),
						'date'       => esc_html__( 'Show date', 'solidbuild' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'solidbuild' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'solidbuild' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = solidbuild_get_locate_template( 'inc/widgets/featured-posts-block/views/widget.php' );
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = solidbuild_get_locate_template( 'inc/widgets/featured-posts-block/views/item.php' );
			 ...
			 */
			$query = apply_filters( 'solidbuild_featured_posts_block_query', $query, $this->instance );
		...
		public function prepare_data( $data, $layout ) {
			...
			$container_template = solidbuild_get_locate_template( 'inc/widgets/featured-posts-block/views/container.php' );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'solidbuild' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'solidbuild' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'solidbuild' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'solidbuild' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'solidbuild' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'solidbuild_register_featured_posts_block_widget' );

	if ( false === function_exists( 'solidbuild_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function solidbuild_register_featured_posts_block_widget() {
			register_widget( 'Solidbuild_Featured_Posts_Block_Widget' );
		}
	}
}
