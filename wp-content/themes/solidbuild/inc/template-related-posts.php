 ...
 * @return array
 */
function solidbuild_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = get_theme_mod( 'related_posts_visible', solidbuild_theme()->customizer->get_default( 'related_posts_visible' ) );
	...
	$post_terms  = array();
	$post_number = get_theme_mod( 'related_posts_count', solidbuild_theme()->customizer->get_default( 'related_posts_count' ) );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = get_theme_mod( $setting_value, solidbuild_theme()->customizer->get_default( $setting_value ) );
	}

	$settings['title_visible']   = $settings['title_length'] ? get_theme_mod( 'related_posts_title', solidbuild_theme()->customizer->get_default( 'related_posts_title' ) ) : false;
	...
	if ( $holder_view_dir ) {
		...
		$block_title_format = apply_filters( 'solidbuild_rp_block_title_format', '<h5 class="related-posts__title title-decoration">%s</h5>' );
		$block_title        = ( $settings['block_title'] ) ? sprintf( $block_title_format, $settings['block_title'] ) : '';
		$accent_color       = get_theme_mod( 'regular_accent_color_1', solidbuild_theme()->customizer->get_default( 'regular_accent_color_1' ) );
		...
		foreach ( $posts as $post ) {
			...
			$image = $utility->media->get_image( apply_filters( 'solidbuild_rp_image_settings', array(
			...
			$title = $utility->attributes->get_title( apply_filters( 'solidbuild_rp_title_settings', array(
			...
			$excerpt = $utility->attributes->get_content( apply_filters( 'solidbuild_rp_excerpt_settings', array(
				'visible'      => $settings['content_visible'],
				'length'       => $settings['content_length'],
				'content_type' => $settings['content_type'],
			) ) );

			$category = $utility->meta_data->get_terms( apply_filters( 'solidbuild_rp_category_settings', array(
				'type'    => 'category',
			...
			$tag = $utility->meta_data->get_terms( apply_filters( 'solidbuild_rp_tag_settings', array(
				'type'      => 'post_tag',
			...
			$author = $utility->meta_data->get_author( apply_filters( 'solidbuild_rp_author_settings', array(
			...
			$date = $utility->meta_data->get_date( apply_filters( 'solidbuild_rp_date_settings', array(
			...
			$comment_count = $utility->meta_data->get_comment_count( apply_filters( 'solidbuild_rp_comment_count_settings', array(
