 ...
 */
function solidbuild_rewrite_comment_item( $_comment, $args, $depth ) {
	global $comment;

	$_comment->solidbuild_comment_list_args = $args;
	$comment = $_comment;
	...
	<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
		<div class="comment-body__holder">
			<?php solidbuild_get_template_part( 'template-parts/comment' ); ?>
		</div>
 ...
 */
function solidbuild_comment_author_avatar( $args = array() ) {
	global $comment;

	if ( ! empty( $comment->solidbuild_comment_list_args['avatar_size'] ) ) {
		$size = $comment->solidbuild_comment_list_args['avatar_size'];
	}
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'solidbuild_comment_author_avatar', get_avatar( $comment, $size ), $args );
}
 ...
 */
function solidbuild_get_comment_author_link( $args = array() ) {
	/**
	 ...
	 * @param array $args   Arguments.
	 */
	return apply_filters( 'solidbuild_get_comment_author_link', sprintf( '<b class="fn h6-style">%s</b>', get_comment_author_link() ), $args );
}
 ...
 */
function solidbuild_get_comment_date( $args = array() ) {
	 ...
	 * @param array  $args   Arguments.
	 */
	return apply_filters( 'solidbuild_get_comment_date', sprintf( '<div class="comment-date"><a href="%3$s" class="comment-date__link"><time datetime="%1$s" class="comment-date__time">%2$s</time></a></div>', get_comment_time( 'c' ), get_comment_date( $format ), get_comment_link() ), $args );
}
 ...
 */
function solidbuild_get_comment_text( $args = array() ) {
	 ...
	 * @param array  $args         Arguments.
	 */
	return apply_filters( 'solidbuild_get_comment_text', $comment_text, $args );
}
 ...
 */
function solidbuild_get_comment_reply_link( $args = array() ) {
	global $comment_depth;
	 ...
	 * @param array  $args  Arguments.
	 */
	return apply_filters( 'solidbuild_get_comment_reply_link', $reply, $args );
}
 ...
 */
function solidbuild_get_comment_link_edit( $args = array() ) {
	global $comment;

	$text = esc_html__( 'Edit', 'solidbuild' );
	 ...
	 * @param array  $args       Arguments.
	 */
	return apply_filters( 'solidbuild_get_comment_link_edit', $link, $comment->comment_ID, $args );
}
